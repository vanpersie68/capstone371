<template>
  <div class="question" :class="isActiveQuestion ? 'active' : ''" @click="handleQuestionClick" id="builder">
    <div class="question-header">
      <div class="question-titles">
        ...
        <input type="text" v-model="question.title" @blur="updateQuestionTitle" class="question-title h2-tb" :style="$i18n.locale === 'ur' ? 'text-align: right;' : 'text-align: left;'
          " />
      </div>
      <div class="opt-container">
        <div class="popup" :style="$i18n.locale === 'ur' ? 'left: 0px;' : 'right: 0px;'"
          ...
          <ul>
            <li>
              <button-base class="secondary min" :icon="'fas fa-clone fa-me'"
                :title="$t('builderQuestion.duplicateQuestion')" @mousedown="
            ...
            <line-base class="dark" />
            <li>
              <button-base class="secondary min red" :title="$t('builderQuestion.deleteQuestion')"
                :icon="'fas fa-trash fa-me'" @mousedown="deleteQuestionFunc" />
            </li>
          </ul>
        </div>
        <button-base class="secondary square" :icon="'fas fa-ellipsis-h fa-lg'" @buttonPress="pressQuestionOptions"
    ...
    <!-- NEWS POST QUESTION -->
    <question-content-articles v-if="question.type === 'News post'" :data="question" />

    <!-- TEXT ENTRY QUESTION -->
    <div v-if="question.type === 'Text entry'" class="question-content">
      <el-button type="primary" style="cursor: pointer" :onclick="'document.getElementById(\'question_' + question.id + '\').click()'
        ">
        {{ $t('builderBlock.uploadImage') }}
      </el-button>
      <el-button type="info" style="cursor: pointer" @click="cleanPhoto(question)">
        {{ $t('g5.cancel') }}
      </el-button>
      <input :id="'question_' + question.id" @change="uploadPhoto($event, question)" type="file" style="display: none"
      ...
      <input type="text" v-model="question.query" @blur="inputTextboxQuestion" />
    </div>
    ...
    <div v-if="question.type === 'Button row'" class="question-content">
      ...
      <div v-for="button in question.buttons" :key="question.buttons.indexOf(button)" class="question-titles">
        <h1>{{ question.buttons.indexOf(button) + 1 }}.</h1>
        <input type="text" v-model="question.buttons[question.buttons.indexOf(button)].text" @blur="(input) =>
            inputTextboxButton(
              question.buttons[question.buttons.indexOf(button)]
            )
          " />
        <h2>{{ $t('builderQuestion.JumpBlock') }}</h2>
        <el-select v-model="question.buttons[question.buttons.indexOf(button)].jumpBlockId
          " placeholder="" @focus="getBlockList()" size="medium" filterable="true" @change="(input) =>
      inputTextboxButton(
        question.buttons[question.buttons.indexOf(button)]
      )
    ">
          <el-option v-for="item in jumpList" :key="item.id" :label="item.title" :value="item.id">
          </el-option>
        </el-select>
      </div>
    </div>
    ...
    <div v-if="question.type === 'Multiple choice'" class="question-content">
      ...
      <div v-for="choice in question.choices" :key="question.choices.indexOf(choice)" class="question-titles">
        <h1>{{ question.choices.indexOf(choice) + 1 }}.</h1>
        <input type="text" style="width: 100%" v-model="question.choices[question.choices.indexOf(choice)].text" @blur="(input) =>
      ...
      <div v-if="question.otherInput" class="question-titles">
        ...
        <input type="text" style="width: 100%" disabled :value="$t('builderQuestion.other')" />
      </div>
    </div>
    <div v-if="question.type === 'Matrix table'" class="question-content">
      <h2>{{ $t('builderQuestion.MatrixTableOptions') }}</h2>
      {{ choicesSort(question) }}
      <el-table ref="singleTable" :data="tableConfig" highlight-current-row style="width: 100%">
        <el-table-column property="name"> </el-table-column>
        <el-table-column v-for="column in columnConfig" :key="column.value" :property="column.value"
          :label="column.label">
          <template slot-scope="scope">
            <div>
              <!-- {{matrixChecked[scope.row.name + column.label]}} -->
              <el-checkbox :key="matrixChecked[scope.row.name + column.label]"
                :checked="matrixChecked[scope.row.name + column.label]" @change="
    ...
    <div v-if="question.type === 'Sliders'" class="question-content" @mousedown="sliderMousedown">
      <h2>{{ $t('pOptionsSliders.Sliders Options') }}</h2>
      {{ choicesSort(question) }}
      <el-table ref="singleTable" :data="tableConfig" :span-method="slidersSpanMethod" highlight-current-row
        style="width: 100%">
        <el-table-column property="name"> </el-table-column>
        <el-table-column v-for="column in columnConfig" :key="column.value" :property="column.value"
          :label="column.label">
          <template slot-scope="scope">
            <div style="height: 50px">
              <el-slider v-model="scope.row.sliderVal" :step="question.typedata.grid" :min="question.typedata.min"
                :max="question.typedata.max" :marks="marks" @change="sliderChange($event, scope)"></el-slider>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div v-if="question.type === 'Groups'" class="question-content">
      ...
      <div class="group-wrapper">
        <Container class="group-left" :should-animate-drop="(src, payload) => true"
          :get-child-payload="(itemIndex) => getChildPayload(itemIndex, 'root')" @drop="groupsDropFa($event)"
          :should-accept-drop="(src, payload) => true">
          <p>{{ $t("surveyTaker.Items") }}</p>
          <Draggable v-for="item in tableConfig" :key="item.name" class="question-titles">
            <div class="ques-item">{{ item.name }}</div>
          </Draggable>
        </Container>
        <div class="group-right">
          <Container v-for="(group, inx) in columnConfig" :key="group.value" class="group-item"
            @drop="groupsDrop(group, $event)" :should-accept-drop="(src, payload) => true"
            :should-animate-drop="(src, payload) => false" :get-child-payload="(itemIndex) => getChildPayloadChil(itemIndex, inx, 'chil')
              ">
    ...
    <div v-if="question.type === 'Rank order'" class="question-content">
      ...
      <Container @drop="onDrop(question, $event)">
        <Draggable v-for="choice in question.choices" :key="question.choices[question.choices.indexOf(choice)].id"
    ...
    <div v-if="question.type === 'Drag and Drop'" class="question-content">
      ...
      <div v-for="choice in question.choices" :key="question.choices.indexOf(choice) + ' Chip'" class="question-titles">
        <h1>{{ question.choices.indexOf(choice) + 1 }}.</h1>

        <input type="text" style="width: 100%" v-model="question.choices[question.choices.indexOf(choice)].text" @unfocus="(input) =>
      ...
      <div v-for="category in question.categories" :key="question.categories.indexOf(category)" class="question-titles">
        <h1>{{ question.categories.indexOf(category) + 1 }}.</h1>

        <input type="text" style="width: 100%" v-model="question.categories[question.categories.indexOf(category)].text
          " @unfocus="(input) =>
      $store.commit('updateValue', {
        parent:
    ...
    <div v-if="question.type === 'Number scale'" class="question-content">
      ...
      <div v-if="question.minTitleOn">
        ...
        <input type="text" v-model="question.minTitle" @blur="inputMinLabelNumberScale" />
      </div>
      <div v-if="question.midTitleOn">
        <h2>{{ $t('builderQuestion.midTitle') }}</h2>

        <input type="text" v-model="question.midTitle" @blur="inputMidLabelNumberScale" />
      </div>
      <div v-if="question.maxTitleOn">
        <h2>{{ $t('builderQuestion.maxTitle') }}</h2>

        <input type="text" v-model="question.maxTitle" @blur="inputMaxLabelNumberScale" />
      </div>
      <line-base class="light" v-if="question.minTitleOn || question.midTitleOn || question.maxTitleOn" />
