<template>
  <div class="panel-base">
    ...
    <div class="options" v-if="editingQuestion != null">
      ...
      <dropdown-base
        :options="editorData.questionTypes"
        :currentType="editingQuestion.type"
        :icons="editorData.questionIcons"
        @input="handleQuestionType"
      />
      <line-base class="dark"></line-base>
      <div class="text-with-button">
        <h2>{{ $t('panelBase.answerReq') }}</h2>
        <toggle-base
          :toggled="editingQuestion.required"
          @handleToggle="toggleRequiredQuestion($store)"
        ></toggle-base>
      </div>
      <line-base class="dark" v-if="editingQuestion != null"></line-base>

      <panel-options-articles
        v-if="editingQuestion.type === 'News post'"
        :question="editingQuestion"
      />

      <panel-options-text-entry
        v-if="editingQuestion.type === 'Text entry'"
        :question="editingQuestion"
      />

      <panel-options-button-row
        v-if="editingQuestion.type === 'Button row'"
        :question="editingQuestion"
      />

      <panel-options-multiple-choice
        v-if="editingQuestion.type === 'Multiple choice'"
        :question="editingQuestion"
      />

      <panel-options-number-scale
        v-if="editingQuestion.type === 'Number scale'"
        :question="editingQuestion"
      />

      <panel-options-drag-and-drop
        v-if="editingQuestion.type === 'Drag and Drop'"
        :question="editingQuestion"
      />
      <panel-options-rank-order
        v-if="editingQuestion.type === 'Rank order'"
        :question="editingQuestion"
      />
      <panel-options-matrix-table
        v-if="editingQuestion.type === 'Matrix table'"
        :question="editingQuestion"
      />
      <panel-options-sliders
        v-if="editingQuestion.type === 'Sliders'"
        :question="editingQuestion"
      />
      <panel-options-groups
        v-if="editingQuestion.type === 'Groups'"
        :question="editingQuestion"
      />
    </div>
  ...
  computed: {
    ...
    editingBlock: function () {
      if (this.editorData.currentBlock == 0) {
        console.log('验证是否是有编辑的目标this.editorData.currentBlock == 0');
        return null
      }
      ...
      if (this.editorData.currentBlock == null) {
        ...
        // console.log('this.editorData.currentBlock == null执行了第二个if');
      }
      // console.log('初始化判断执行到这里','block[0]',block[0]);
      return block[0]
    },
    /**
    ...
    editingQuestion: function () {
      if (
        ...
        // console.log('this.editingBlock',this.editingBlock);
        // console.log('this.editingBlock.questionData.currentQuestion',this.editingBlock.questionData.currentQuestion);
        return null
      }
