<template>
  <div class="question-box" :style="color">
    ...
    <el-table
      ...
      style="width: 100%"
    >
      <el-table-column property="name" min-width='200'> </el-table-column>
      <el-table-column
        ...
        <template slot-scope="scope">
          <div style="text-align: center">
            ...
            <el-checkbox
              ...
              @change="handleMatrixChange(scope.row.name, column.label, $event)"
            ></el-checkbox>
          </div>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    checkValidity() {
      if (this.questionData.required) {
        if (Object.values(this.matrixChecked).filter(Boolean).length === 0) {
