<template>
    <div class="out-box">
      ...
      <el-row style="margin: 10px">
        ...
        <!--send-->
        <!-- <el-col :style="colWidth" v-show="sendShow">
                  <div class="clickItem-send" :title="sendTitle">
                    <span
                      class="iconfont clickItem-icon-send"
                      :style="iconColorSend"
                      @click="clickItem('Send')"
                      >
                      <el-icon class="el-icon-send">
                    </el-icon>
                    </span>
                    <span class="clickItem-title-send" :style="textColorSend">{{
                      this.questionData.articleSends
                    }}</span>
                  </div>
                </el-col> -->
                ...
                <div class="clickItem-send" :title="sendTitle">
                  ...
                  <span class="clickItem-title-send" :style="textColorSend">{{ questionData.articleSends }}</span>
                </div>
              </el-col>
  ...
  export default {
    ...
    methods: {
      ...
      checkSend() {
        for (let i = 0; i < this.countItems; i++) {
          if (this.answerObj[i].title === 'Send') {
            this.answerObj[i].answerText = 'selected'
          }
        }
      },
      ...
      checkShow() {
        ...
        this.sendShow = this.questionData.articleSendsOn
      ...
      clickItem(item) {
        ...
        if (item === 'Send') {
          this.inputSendShow = !this.inputSendShow
          if (this.inputSendShow === true) {
            this.questionData.articleSends++
            this.changeSendColor(true)
            this.checkLike()
            this.sendResult()
          } else {
            this.questionData.articleSends--
            this.changeSendColor(false)
            this.cancelCheckSend()
            this.sendResult()
          }
        }
      ...
      changeSendColor(flag) {
        if (flag === true) {
          this.iconColorSend = 'color: #FFD700;background: #FFFFCC;'
          this.textColorSend = 'color: #FFD700;'
          this.sendTitle = 'not Send'
        } else {
          this.iconColorSend = 'color: #7f7f7f;background: #FFFFFF;'
          this.textColorSend = 'color: #7f7f7f;'
          this.sendTitle = 'Send'
        }
      },
      ...
      addClickItems() {
        ...
        if (this.questionData.articleSendsOn === true) {
          this.clickItems.push('Send')
          this.countItems++
        }
  ...
  @media (min-width: 1130px) {
    ...
    .clickItem-icon-send:hover {
      font-size: 15px;
      color: #FFD700;
      background: #FFFFCC;
    }
  
    .clickItem-title-send {
      font-family: inherit;
      margin-left: 5px;
      color: #7f7f7f;
      font-size: 15px;
    ...
    .clickItem-icon-send:hover + .clickItem-title-send {
      color: #e8d722;
    }
