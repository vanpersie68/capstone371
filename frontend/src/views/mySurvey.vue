<template>
  <div>
    ...
    <div v-else class="main">
      ...
      <el-input
        :placeholder="$t('g5.Search by title')"
        suffix-icon="el-icon-search"
        style="display: inline-block; width: 180px; margin-right: 12px"
        v-model="keyword"
      >
      </el-input>
      <el-select
        :placeholder="$t('g5.Select by language')"
        style="display: inline-block; width: 180px; margin-right: 12px"
        v-model="shLang"
      >
        <el-option
      ...
      <el-select
        :placeholder="$t('g5.Select by Survey Status')"
        style="display: inline-block; width: 180px; margin-right: 12px"
        v-model="shStatus"
      >
        <el-option
        ...
        <tr v-for="survey in pageSurveys" :key="survey.id">
          ...
          <td>
            <span v-if="survey.status === 0"
              ><i class="fas fa-circle" style="color: #1947e5"></i>
              {{ $t('g5.Saved') }}
            </span>
            <span v-if="survey.status === 1"
              ><i class="fas fa-circle" style="color: #18e200"></i>
              {{ $t('g5.Published') }}
            </span>
          ...
          <td style="padding-right: 0">
            ...
            <a
              ...
              @click="showlinkDialogVisible(survey.code)"
              >{{ $t('g5.Share') }}</a
            >
            ...
            <a
              :href="getConfigurationExportUrl(survey.id)"
              class="action-btn"
              >{{ $t('g5.ms-export-conf') }}</a
            >
            <span>
              <select class="action-btn" v-model="cloneLanguage">
                <option
                  v-for="[key, value] in Object.entries(languageOptions)"
                  :value="key"
                  :id="key"
                >
                  {{ value }}
              ...
              <a
                class="action-btn"
                @click="cloneSurvey(survey.id, survey.name)"
                >{{ $t('g5.Clone') }}</a
              >
            </span>
            <a
            ...
            <a
              class="action-btn"
              @click="collaborator(survey.id, survey.name)"
      ...
      <div v-else>
        <el-row>
          <el-col
            :span="6"
            v-for="(survey, index) in pageSurveys"
            :key="survey.id"
            :style="{ padding: '10px' }"
          >
              ...
              <div style="padding: 14px">
                ...
                <div class="bottom clearfix card-btns">
                  ...
                  <el-button
                    ...
                    v-if="survey.status === 1"
                    @click="showlinkDialogVisible(survey.code)"
                    >{{ $t('g5.Share') }}</el-button
                  >
                  <el-button
                    type="text"
                    class="action-btn"
                    v-if="survey.status === 2"
                    >{{ $t('g5.Share') }}</el-button
                  >
                  <el-button
                    type="text"
                    @click="getStatUrl(survey.id)"
                    class="action-btn"
                    >{{ $t('g5.ms-view-stats') }}</el-button
                  >
                  <el-button
                    type="text"
                    @click="getResponseExportUrl(survey.id)"
                    class="action-btn"
                    >{{ $t('g5.Export') }}</el-button
                  >
                  <el-button
                    type="text"
                    :href="getConfigurationExportUrl(survey.id)"
                  ...
                  <el-button
                    ...
                    @click="endSurvey(survey.id)"
                    v-else-if="survey.status === 1"
                    >{{ $t('g5.End Survey') }}</el-button
                  >
                  <el-button
                    type="text"
                    class="action-btn-mute"
                    v-if="survey.status === 2"
                    >{{ $t('g5.Finished') }}</el-button
                  >
                  <el-button
                    type="text"
                    class="action-btn"
